...
public class CricketController {
	...
	public ResponseEntity<List<Cricketer>> getPersons() {
		...
		ResponseEntity<Cricketer[]> personListResponse = restTemplate.getForEntity(PERSONS_URL, Cricketer[].class);
		List<Cricketer> persons = Arrays.asList(personListResponse.getBody());

		return new ResponseEntity<>(persons, HttpStatus.OK);
	}

	@GetMapping(value = "/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Cricketer> getPersonById(@PathVariable("id") final long id) {

		RestTemplate restTemplate = new RestTemplate();
		ResponseEntity<Cricketer> personListResponse = restTemplate.getForEntity(ID_PATH_URL, Cricketer.class, id);
